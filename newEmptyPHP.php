...
// Function to search the table based on input
    ...
    // Assume worker_ids_arr is already available and contains the IDs of the supervisor's workers
var worker_ids_arr = []; // This should be filled by the Firebase data fetch

function searchTable() {
    var input, filter, table, tr, tdName, tdID, txtValueName, txtValueID;
    input = document.getElementById("searchInput");
    filter = input.value.toUpperCase().trim(); // Use toUpperCase for case-insensitive comparison
    ...
    for (var i = 1; i < tr.length; i++) {
        ...
        if (tdName && tdID) {
            txtValueName = tdName.textContent.toUpperCase() || tdName.innerText.toUpperCase();
            txtValueID = tdID.textContent || tdID.innerText;
            let workerID = parseInt(tdID.textContent);

            // Check if worker is one of the supervisor's and if the filter matches the name or ID
            if (worker_ids_arr.includes(workerID) && (txtValueName.indexOf(filter) > -1 || txtValueID.indexOf(filter) > -1)) {
                tr[i].style.display = "";
            } else {
...
// Fetching and setting supervisor's worker IDs (make sure this part is done before searchTable is required)
let supervisorId = JSON.parse('<?php echo json_encode($_SESSION["record"]); ?>');
let supervisorRef = firebase.database().ref(`supervisors/${supervisorId}/workerIDs`);

supervisorRef.once('value', function(snapshot) {
    let worker_ids = snapshot.val();
    worker_ids_arr = worker_ids.split(',').map(item => parseInt(item.trim()));
});
